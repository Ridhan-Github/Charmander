...
async def media(c, m):
    ...
    file_name = await c.ask(chat_id=m.from_user.id, text="**Nᴏᴡ Sᴇɴᴅ Nᴇᴡ Fɪʟᴇ Nᴀᴍᴇ Oʀ Sᴇɴᴅ /cancel Tᴏ Sᴛᴏᴘ**", filters=filters.text)
    ...
    try:
        file_location = await m.download(
                            ...
                            progress_args=("╭─⌊ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ⌉", start_time, send_message)
                        )
    except Exception as e:
        logger.error(e)
    ...
    if as_file:
        try:
            await m.reply_document(
                ...
                thumb=thumbnail,
                progress=progress_bar,
                progress_args=("╭─⌊ᴜᴘʟᴏᴀᴅɪɴɢ⌉", start_time, send_message)
            )
...
async def notify(m, time_gap):
    ...
    await m.reply_text("__Yᴏᴜ Cᴀɴ Usᴇ Mᴇ Nᴏᴡ 🚀__")
